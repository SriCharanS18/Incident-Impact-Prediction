...
input=np.array([[number, fun1(incident_state), opdened_by, opened_at, sys_updated_by, sys_updated_at, resolved_at, resolved_by, closed_at,
      fun7(subcategory)]])
...
def fun7(subcategory):
    ...
    openby=['opened_by '+str(i) for i in range(2,224,1)]
value1=np.array([[j] for j in range(0,224,1)])
value1=scaler.fit_transform(value1)
dic1={ i:j for i,j in zip(openby,value1) }
...
def main():
    ...
    opened_by = st.number_input("opened_by", min_value=0,step=1)
    opened_at= st.number_input("opened_at", min_value=0,step=1)
    sys_updated_by= st.number_input("sys_updated_by", min_value=0,step=1)
    sys_updated_at= st.number_input("sys_updated_at", min_value=0,step=1)
    subcategory=st.selectbox("Subcategory ?",(['Subcategory '+str(i) for i in range(2,254,1)]))
    resolved_at= st.number_input("resolved_at", min_value=0,step=1)
    resolved_by= st.number_input("resolved_by", min_value=0,step=1)
    closed_at= st.number_input("closed_at", min_value=0,step=1)
    result =""
    ...
    if st.button("Predict"):
        result = prediction(number, incident_state, opened_by, opened_at, sys_updated_by, sys_updated_at, subcategory, resolved_by, resolved_at, closed_at)
